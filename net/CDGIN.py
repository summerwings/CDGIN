...
from net.CL import CL_MLP

class ModuleTimestamping(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=1, dropout=0.0, layers = 6, causal_conv = True):
        super().__init__()
...
class ModuleTransformer(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_heads, dropout=0.1, t_k = 9):
        super().__init__()

        self.MaxPool = nn.AdaptiveMaxPool1d((1))
        self.AvgPool = nn.AdaptiveAvgPool1d((1))

        self.Conv1d = nn.Conv1d(1, 1, kernel_size=t_k, padding=t_k//2)
    ...
    def forward(self, x):
        ...
        max = self.MaxPool(x)
        max = max.permute(1,2,0) # b, h, t
        avg = self.AvgPool(x)
        avg = avg.permute(1,2,0) # b, h, t

        max = self.Conv1d(max)
        avg = self.Conv1d(avg)

        att = max + avg
        att = self.a(att) # b, h, t
        att = att.permute(2, 0, 1)  # t, b, h
        attn_matrix = att
        x_attend = x * att
...
class ModuleFusion(nn.Module):

    def __init__(self, hidden_dim, graph_n = 2, dropout=0.1):
        super().__init__()

        self.MaxPool = nn.AdaptiveMaxPool2d((2,1))
        self.AvgPool = nn.AdaptiveAvgPool2d((2,1))

        self.MLP = nn.Sequential(nn.Linear(graph_n, graph_n//2),nn.ReLU(), nn.Dropout(dropout), nn.Linear(graph_n//2, graph_n))
    ...
    def forward(self, x):
        ...
        max = self.MaxPool(x).squeeze(-1) # t, b, g
        avg = self.AvgPool(x).squeeze(-1) # t, b, g

        max = self.MLP(max)
        avg = self.MLP(avg)

        att = max + avg
        att = self.a(att).unsqueeze(-1) # t, b, g, c
...
class ModelCDGIN(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes, num_heads, num_layers, sparsity, dropout=0.5, cls_token='sum', readout='garo', garo_upscale=1.0, more_stat_des = True, graph_n = 2):
        super().__init__()
        ...
        self.gnn_layers2 = nn.ModuleList()
        self.fusions = nn.ModuleList()
        self.readout_modules = nn.ModuleList()
        self.readout_modules2 = nn.ModuleList()
        ...
        for i in range(num_layers):
            ...
            self.gnn_layers2.append(LayerGIN(hidden_dim, hidden_dim, hidden_dim))
            self.readout_modules.append(readout_module(hidden_dim=hidden_dim, input_dim=input_dim, dropout=0.1))
            self.readout_modules2.append(readout_module(hidden_dim=hidden_dim, input_dim=input_dim, dropout=0.1))
            self.fusions.append(ModuleFusion(hidden_dim=hidden_dim, graph_n=graph_n, dropout=0.1))
            self.transformer_modules.append(ModuleTransformer(hidden_dim, 2*hidden_dim, num_heads=num_heads, dropout=0.1))
            self.linear_layers.append(nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, num_classes)))

        self.activation = torch.nn.Sigmoid()

        self.more_stat_des = more_stat_des

        self.cl = CL_MLP(hidden_dim)
    ...
    def forward(self, v, a, a_var, t, sampling_endpoints):
        ...
        if self.more_stat_des:
            a_var = self._collate_adjacency(a_var, self.sparsity)

        for layer, (G, G_var, F, R, R_var, T, L) in enumerate(zip(self.gnn_layers, self.gnn_layers2, self.fusions, self.readout_modules, self.readout_modules2, self.transformer_modules, self.linear_layers)):
            h_cor = G(h, a)
            h_bridge = rearrange(h_cor, '(b t n) c -> t b n c', t=num_timepoints, b=minibatch_size, n=num_nodes)
            if self.more_stat_des:
                h_var = G_var(h, a_var)
                h_bridge_var = rearrange(h_var, '(b t n) c -> t b n c', t=num_timepoints, b=minibatch_size, n=num_nodes)

            h_readout, node_attn = R(h_bridge, node_axis=2)
            h_readout_var, node_attn = R_var(h_bridge_var, node_axis=2)

            print('h r 1',h_readout.size(), 'h r 2', h_readout_var.size())
            _, _, sim = self.cl(h_readout, h_readout_var)
            h_fusion = torch.stack((h_readout, h_readout_var), dim=2)
            h_fusion = F(h_fusion)
            ...
            ortho_latent = rearrange(h_bridge_var, 't b n c -> (t b) n c')
            matrix_inner = torch.bmm(ortho_latent, ortho_latent.permute(0,2,1))
            reg_ortho += (matrix_inner/matrix_inner.max(-1)[0].unsqueeze(-1) - torch.eye(num_nodes, device=matrix_inner.device)).triu().norm(dim=(1,2)).mean()
        ...
        return logit, attention, latent, reg_ortho, sim
